...
[![Deploy Dashboard to GitHub Pages](https://github.com/kleinnconrad/stock-predictor/actions/workflows/04_deploy_pages.yml/badge.svg?branch=main)](https://github.com/kleinnconrad/stock-predictor/actions/workflows/04_deploy_pages.yml)
## Table of Contents
...
* **Cross-Validation Accuracy Safeguard (TimeSeriesSplit)**: To prevent data leakage and look-ahead bias common in financial forecasting, the engine strictly employs a manual **2-Fold TimeSeriesSplit** without shuffling. This ensures the model is only ever trained on past data and evaluated on strictly future, non-overlapping windows. Before making a final prediction, the ticker must achieve a minimum **CV Accuracy of 65%** `(True Positives + True Negatives) / Total Observations` across the out-of-sample test folds. Additionally, the model must satisfy the **Strict Confusion Matrix Rule**: `True Positives > False Negatives AND True Negatives > False Positives`. If the algorithm cannot accurately model the historical patterns above this threshold, or fails the strict confusion matrix rule, it rejects the ticker as "NOT_UP" regardless of the current day's prediction.
...
gantt
    ...
    section Fold 1
    Training Data :active, 2016-01-01, 2017-01-01
    Test Data     :crit, 2017-01-01, 2018-01-01
...
### Additional Metrics (Dashboard Display)
While the prediction models rely on time-series analysis and fundamental rules, the engine also extracts additional metrics to provide broader context on the dashboard. These metrics are:

* **P/E Ratio (KGV):** $\text{P/E} = \frac{\text{Current Stock Price}}{\text{Earnings Per Share (EPS)}}$. The Price-to-Earnings ratio measures a company's current share price relative to its per-share earnings. A high P/E could mean that a stock's price is high relative to earnings and possibly overvalued, or that investors expect high growth rates in the future. Conversely, a low P/E might indicate that the current stock price is low relative to earnings (undervalued). 
  * **Note on Dimensionality:** The P/E ratio represents a point-in-time snapshot (dimensional data) rather than a continuous, stationary time-series. Because the Step 1 machine learning model requires strictly stationary, continuous time-series data to evaluate momentum, the P/E ratio is explicitly excluded as a predictor in Step 1. Furthermore, it is only calculated from time to time on historic data.
* **Beta Factor:** $\beta = \frac{\text{Covariance}(R_s, R_m)}{\text{Variance}(R_m)}$. A measure of a stock's volatility in relation to the overall market. A Beta greater than 1 indicates the stock is theoretically more volatile than the market, while a Beta less than 1 indicates it is less volatile. Note: Beta is explicitly excluded as a predictor variable in Step 1 due to high collinearity with the target stock's inherent volatility baseline and its nature as a static, market-relative snapshot.
* **Liquidity (Avg Daily Traded Value):** $\text{Liquidity (€M)} = \frac{\text{Average Daily Volume} \times \text{Latest Stock Price}}{1,000,000}$. This provides an estimate of the average monetary value traded daily (in Millions of Euros). It is crucial for assessing orderbook slippage; low liquidity indicates that large buy or sell orders might significantly impact the stock price, making it difficult to enter or exit positions cleanly. Note: Absolute liquidity is explicitly excluded as a predictor variable in Step 1 due to its severe collinearity with market capitalization and the absolute stock price.
...
The pipeline requires synchronized data retrieval to ensure reproducible predictions between local executions and cloud-based GitHub Actions. Previous versions experienced timezone-induced offsets because intraday pricing data was fetched depending on the execution time.

To maintain stability, the engine standardizes the current date to the `Europe/Berlin` timezone and programmatically drops pricing data corresponding to the current calendar day. The system uses only fully closed, historical trading candles for all macroeconomic and target stock analyses.

> [!NOTE]
> Because the system drops the current day's live trading data, real-time prices during market hours will not influence the prediction until the subsequent calendar day.

---

## Dependency Management

The project utilizes the "Lockfile Pattern" via `pip-tools` to guarantee reproducible builds and eliminate transient dependency conflicts.

* **Top-Level Requirements**: `requirements.in` defines the direct, top-level packages required by the engine.
* **Lockfile Compilation**: `requirements.txt` acts as the compiled lockfile. It is automatically generated by running `pip-compile requirements.in`. You should **never manually edit** `requirements.txt`.
* **Automated Updates**: Dependabot is configured to read the `requirements.in` file, automatically run `pip-compile` to safely resolve updates, and group all Python dependency changes into a single, conflict-free Pull Request.

---

## Release Management

This repository uses [Google's Release Please](https://github.com/googleapis/release-please-action) to automate changelog generation, the creation of GitHub releases, and version bumps.

### How to trigger a release
Release Please is triggered automatically via GitHub Actions whenever code is pushed to the `main` branch. To ensure the workflow operates correctly, all commits must follow the **Conventional Commits** specification. 

- **`feat:`** triggers a Minor version bump and adds the feature to the `CHANGELOG.md`.
- **`fix:`** triggers a Patch version bump and adds the bug fix to the `CHANGELOG.md`.
- **`chore:` / `ci:` / `docs:`** will not trigger a new release by themselves, but are used for internal repository tasks.

When a `feat:` or `fix:` commit is pushed to `main`, the bot will automatically open a **Release Pull Request**. Once that PR is approved and merged, the release is officially published, tagged, and the `CHANGELOG.md` is updated on the `main` branch.
