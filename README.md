...
* **Beta Factor:** `Covariance(Stock Return, Market Return) / Variance(Market Return)`. A measure of a stock's volatility in relation to the overall market. A Beta greater than 1 indicates the stock is theoretically more volatile than the market, while a Beta less than 1 indicates it is less volatile.
* **Liquidity (Avg Daily Traded Value):** `(Average Daily Volume * Latest Stock Price) / 1,000,000`. This provides an estimate of the average monetary value traded daily (in Millions of Euros). It is crucial for assessing orderbook slippage; low liquidity indicates that large buy or sell orders might significantly impact the stock price, making it difficult to enter or exit positions cleanly.
